...
- (void) drawConnectNote2:(CGContextRef)context andYtop:(float)ytop andTopStaff:(WhiteNote *)topStaff
{
    ...
    int radian1;
    int v = abs(_connectNoteWidth2)/width;
    switch(v) {
        case 0:
            radian1 = 4;
            break;
    ...
    if (direct == StemDown) {
        for (int noteindex = 0; noteindex < notedata_len; noteindex++) {
            ...
            if (notedata_len == 1) {
                if (note->next == 1) {
                    ynote = ytop + [topStaff dist:(note->whitenote)] * NoteHeight/2 - NoteHeight/4;
                    [aPath moveToPoint:CGPointMake(-NoteWidth/4, ynote)];
                    [aPath addQuadCurveToPoint:CGPointMake(_connectNoteWidth2 - 5 + NoteWidth/2, ynote) controlPoint:CGPointMake((NoteWidth/4 + _connectNoteWidth2 - 5)/2, ynote - radian1)];
                    [aPath stroke];
                }
            }else if(notedata_len == 2){
                if (noteindex == 1) {
...
- (void) drawConLine:(CGContextRef)context andYtop:(float)ytop andTopStaff:(WhiteNote *)topStaff
{
    ...
    int leftDirect, rightDirect, direct, ynote = 0, ynote1 = 0;
    ...
    if (_conLineWidth > 0 && conLine > 0) {
        ...
        if (direct == StemDown) {
            ...
            if (rightDirect == StemDown) {
                ynote1 = ytop + [topstaff dist:(data->whitenote)] * NoteHeight/2 - NoteHeight/2;
            } else if (rightDirect == StemUp) {
                ynote1 = ytop + [topstaff dist:(data->whitenote)] * NoteHeight/2 + NoteHeight/4;
            }else{
                if (direct == StemDown){
                    ynote1 = ytop+ [topstaff dist:(data->whitenote)] * NoteHeight/2 - NoteHeight;
                }else if (direct == StemUp){
                    ynote1 = ytop+ [topstaff dist:(data->whitenote)] * NoteHeight/2 + 2*NoteHeight;
                }else{
                    ynote1 = ytop+ [topstaff dist:(data->whitenote)] * NoteHeight/2 - NoteHeight;
                }
            }
            ...
            if (rightDirect == StemDown) {
                ...
                NoteData *note = [_conLineChord getNotedata];
        ...
        if (direct == StemDown) {
            ...
            ynote = ytop + [topStaff dist:[stem top]] * NoteHeight/2 + NoteHeight;
            [aPath moveToPoint:CGPointMake(NoteWidth, ynote)];
            [aPath addQuadCurveToPoint:CGPointMake(_conLineWidth+NoteWidth/2, ynote-radian2) controlPoint:CGPointMake((NoteWidth/2+_conLineWidth+NoteWidth/2)/2, ynote-radian2)];
            [aPath stroke];
        }
        ...
        if (direct == StemDown) {
            ...
            if (direct == StemDown) {
                ...
                ynote =  ytop + [topStaff dist:[stem top]] * NoteHeight/2 - 3*NoteHeight;
            }
